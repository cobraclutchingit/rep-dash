...
export async function GET(request: NextRequest) {
  try {
    ...
    const whereClause: Prisma.OnboardingTrackWhereInput = {
      isActive: true,
    };
      ...
      include: {
        steps: {
          include: {
            progress: {
              where: {
                OR: [{ startedAt: { gte: startDate } }, { completedAt: { gte: startDate } }],
              },
            },
            _count: {
              select: {
    ...
    const analyticsData = await Promise.all(
      tracks.map(async (track) => {
        ...
        const stepStats = track.steps.map((step) => {
          ...
          const completedProgresses = step.progress.filter(
          ...
          if (completedProgresses.length > 0) {
            const totalTime = completedProgresses.reduce((total, p) => {
              // Check if startedAt and completedAt are not null before using them
        ...
        const trackCompletionRate =
          activeUsers > 0 ? Math.round((usersCompletedAll / activeUsers) * 100) : 0;
    ...
    return NextResponse.json({ error: 'Failed to fetch onboarding analytics' }, { status: 500 });
  }
}
