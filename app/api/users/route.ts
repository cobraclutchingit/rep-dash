...
export const GET = createApiHandler({
  GET: {
    ...
    handler: async (req: NextRequest, { session }) => {
      ...
      // Add filters
      if (role) where.role = role as any; // Simplify to avoid type issues
      if (position) where.position = position as any; // Simplify to avoid type issues
      ...
      return createSuccessResponse({ users, totalCount, totalPages }, 200, {
        page,
        pageSize,
        totalCount,
        totalPages,
