...
export default function EventAgendaItem({ event }: EventAgendaItemProps) {
  ...
  return (
    <div
      className={`border-l-2 py-2 pl-3 ${getEventColor(event.eventType)} ${getEventBgColor(event.eventType)}`}
    >
      ...
      <div className={`text-xs ${getEventTextColor(event.eventType)} flex items-center`}>
        ...
        {event.isBlitz && (
          <span className="ml-2 rounded bg-orange-100 px-1.5 py-0.5 text-[10px] text-orange-800 dark:bg-orange-900/20 dark:text-orange-300">
            BLITZ
          </span>
        )}
      </div>
      ...
      {event.description && <div className="mt-1 line-clamp-1 text-xs">{event.description}</div>}
    </div>
  );
}
