...
export default function CalendarAnalytics({ eventTypeStats, eventsByDate }: CalendarAnalyticsProps) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-sm font-medium mb-2">Events by Type</h3>
        {totalEvents > 0 ? (
            ...
            {sortedStats.map((item) => (
              <div key={item.type} className="space-y-1">
                <div className="flex justify-between text-sm">
                  ...
                  <span>{item.count} ({Math.round((item.count / totalEvents) * 100)}%)</span>
                </div>
      ...
      <div>
        <h3 className="text-sm font-medium mb-2">Events Created (Last 30 Days)</h3>
              ...
              {Object.entries(eventsByDay).map(([date, count], index) => {
                ...
                return (
                  <div key={date} className="flex-1 flex flex-col items-center">
                    <div
                      className="w-full max-w-[8px] mx-auto bg-primary rounded-t"
                      style={{
