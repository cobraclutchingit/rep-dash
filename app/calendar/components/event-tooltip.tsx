...
export default function EventTooltip({ event, position, onClose }: EventTooltipProps) {
  ...
  return (
    <div
      ref={tooltipRef}
      className="absolute z-50 bg-card border border-border rounded-md shadow-lg p-3 min-w-60 max-w-72"
      style={{
        top: tooltipPos.top,
    ...
    >
      ...
      <div className="text-xs text-muted-foreground mb-2 flex items-center">
        <span className={`inline-block h-2 w-2 rounded-full mr-1 ${getEventTypeColor(event.eventType)}`}></span>
        <span>{formatEventType(event.eventType)}</span>
        {event.isBlitz && (
          <span className="ml-2 px-1.5 py-0.5 bg-orange-100 dark:bg-orange-900/20 text-orange-800 dark:text-orange-300 rounded-sm text-[10px]">
            BLITZ
          </span>
        )}
      </div>
      ...
      <div className="text-xs mb-2">
        <div className="flex items-start">
          <CalendarIcon className="mr-1 mt-0.5 flex-shrink-0" />
          <div>
      ...
      {event.location && (
        <div className="text-xs flex items-start mb-2">
          <LocationIcon className="mr-1 mt-0.5 flex-shrink-0" />
          <div>{event.location}</div>
        </div>
      )}
      
      {event.description && (
        <div className="text-xs mt-2 border-t pt-2 line-clamp-2">
          {event.description}
        </div>
      )}
    </div>
