...
export default function EventDetailsModal({ isOpen, onClose, _userId }: EventDetailsModalProps) {
  const { data: session } = useSession();
  const { selectedEvent, setIsCreatingEvent, deleteEvent } = useCalendar();
  ...
  return (
    <div className="bg-background/50 fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="bg-card w-full max-w-md rounded-lg shadow-lg">
        ...
        <div className="space-y-4 p-6">
          <div className="flex items-center space-x-2">
            ...
            {selectedEvent.isBlitz && (
              <span className="ml-2 rounded-sm bg-orange-100 px-2 py-0.5 text-xs text-orange-800 dark:bg-orange-900/20 dark:text-orange-300">
                BLITZ
              </span>
            )}
          </div>
          ...
          {selectedEvent.attendees.length > 0 && (
            <div className="mt-4 border-t pt-4">
              <h3 className="mb-2 text-sm font-medium">Attendees</h3>
              <div className="max-h-40 space-y-2 overflow-y-auto">
                {selectedEvent.attendees.map((attendee) => (
                  <div key={attendee.id} className="flex items-center">
                    <div className="bg-primary/10 text-primary mr-2 flex h-6 w-6 items-center justify-center rounded-full text-xs">
          ...
          <div className="text-muted-foreground mt-4 flex items-center border-t pt-4 text-xs">
