...
export function CommunicationProvider({ children }: { children: React.ReactNode }) {
  ...
  const linkCategories = useMemo(() => {
    const categories = links
      ...
      .filter(
        (category, index, self) => category !== null && self.indexOf(category) === index
      ) as string[];
    return categories;
  }, [links]);
  ...
  }, [session, fetchCommunicationData]);
  ...
  return (
    <CommunicationContext.Provider value={contextValue}>{children}</CommunicationContext.Provider>
  );
}
