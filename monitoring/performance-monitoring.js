...
const ENABLE_PERFORMANCE_MONITORING = process.env.NEXT_PUBLIC_ENABLE_PERFORMANCE_MONITORING === 'true';

// Custom performance reporter function
const reportVitals = ({ name, delta, id, value }) => {
...
const getThresholdForMetric = (name) => {
  switch (name) {
    case 'CLS': return 0.1; // Good: 0-0.1, Poor: >0.25
    case 'FID': return 100; // Good: 0-100ms, Poor: >300ms
    case 'LCP': return 2500; // Good: 0-2.5s, Poor: >4s
    case 'FCP': return 1800; // Good: 0-1.8s, Poor: >3s
    case 'TTFB': return 800; // Good: 0-800ms, Poor: >1800ms
...
const reportCustomPerformance = (name, duration) => {
  // Skip if performance monitoring is disabled
  if (!ENABLE_PERFORMANCE_MONITORING) return;
  
  // Log to console in development
  if (process.env.NODE_ENV === 'development') {
    console.log(`Custom Performance: ${name}`, {
...
export const trackResourceLoading = () => {
  if (!ENABLE_PERFORMANCE_MONITORING || typeof window === 'undefined' || !window.performance) return;
  ...
  window.addEventListener('load', () => {
    ...
    setTimeout(() => {
      ...
      resources
        ...
        .forEach(resource => {
          reportCustomPerformance(`slow_resource_${resource.initiatorType}`, 
...
export const measureApiCall = async (endpoint, method, callback) => {
